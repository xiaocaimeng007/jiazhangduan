...
public class ShouCangActivity extends BaseTestActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setTextview(tvTitle,"我的收藏");
        final LinearLayoutManager layoutManager = new LinearLayoutManager(this);
        rvGuanzhu.setLayoutManager(layoutManager);

        RequestService.doGet(Urls.getFavorite(SharedPrefUtils.getString(getApplicationContext(),SharedPrefUtils.APP_USER_ID),"1") , RESULT_ALL_DATA , this);
    }
            ...
            case RESULT_ALL_DATA:
                try {
                    if (resultBean.isSuccess()){
                        ...
                        ShouCangAdapter adapter = new ShouCangAdapter(activity , favoriteBean.getData());
    ...
    class ShouCangAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder>{
        private Context context ;
        private LayoutInflater inflater;
        private ArrayList<FavoriteBean.Data> datas;
        public ShouCangAdapter(Context context , ArrayList<FavoriteBean.Data> datas){
        ...
        public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
            final FavoriteBean.Data data = datas.get(position);
            ((ViewHodler1)holder).itemView.setTag(data);
            ((ViewHodler1)holder).tv_name.setText(data.getTitle());
            ((ViewHodler1)holder).tv_text.setVisibility(View.GONE);
            holder.itemView.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    Bundle bundle = new Bundle();
                    bundle.putString("title", data.getTitle());
                    bundle.putString("content_cid" ,data.getCid()+"");
                    bundle.putString("_context" ,"");
